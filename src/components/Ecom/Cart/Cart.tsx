...
const Cart = ({ items, totalPrice, checkoutState, errorMessage }: CartType) => {
  ...
  function onQuantityChanged(
    ...
    e.target.value="3";
      ...
      showAlert(max_quantity);
    }

    dispatch(updateQuantity({ id, quantity, max_quantity }));
  }
  function showAlert(max_quantity: number) {
    Swal.fire({
      title: "error",
      text: `sorry but the maximum quantity is ${max_quantity}`,
      icon: "error",
                ...
                <tr key={product.id} className="w-100">
                  ...
                  <td>
                    <select
                      ...
                      onChange={(e) => {
                        onQuantityChanged(e, product.id, product.max_quantity);
                      }}
                    >
